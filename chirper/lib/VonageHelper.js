// Filename: VonageHelper.js

const CallLog = require('../models/CallLog');
const NexmoHelperError = require('./VonageHelperError');
...
class VonageHelper {
  ...
  async handleMessage(message) {
    if (!message.tokens || message.tokens.length === 0) {
      throw new VonageHelperError('Message does not contain any valid command.');
    }
  ...
  async _conferenceIntent(message) {
    if (message.tokens.length < 2) {
      throw new VonageHelperError('"call" command requires tokens for call participants. For example, phone numbers.');
    }
    ...
    this._conferenceUsers(usersToDialIn, async (callResults) => {
      for (const result of callResults) {
        await this._sendCallLogToTelegram(message.channel, result);
      }
  ...
  _conferenceUsers(users, callback, message) {
    ...
    Object.keys(users).forEach((userId) => {
      this._makeCall(conferenceId, userId, async (result) => {
        ...
        await callLog.save().catch(err => console.error('Error saving call log:', err));
  ...
  _makeCall(conferenceId, phoneNumber, callback, message) {
    ...
    this.vonage.voice.createOutboundCall({
      ...
      from: { 
        type: 'phone', 
        number: this.config.CALLER_ID[0] 
      },
  ...
  async _sendCallLogToTelegram(channel, callLog) {
    const message = `<b>Call Log:</b>\n` +
                    `Conference ID: ${callLog.conferenceId}\n` +
                    `Phone Number: ${callLog.phoneNumber}\n` +
                    `Status: ${callLog.status}\n` +
                    `DTMF Input: ${callLog.dtmfInput.join(', ')}\n` +
                    `Responses: ${callLog.responses.join(', ')}`;
