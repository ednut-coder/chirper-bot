...
const VonageHelper = require('./VonageHelper');
const VonageHelperError = require('./VonageHelperError');
...
bot.command('help', (ctx) => ctx.reply(VonageHelper.HELP_MSG));
...
bot.on('message:text', async (ctx) => {
  ...
  if (parsedMessage.isCommand) {
    const vonageHelper = new VonageHelper(bot, vonage, config);

    try {
      await vonageHelper.handleMessage({ ...parsedMessage, channel: message.chat.id });
    } catch (error) {
      if (error instanceof VonageHelperError) {
